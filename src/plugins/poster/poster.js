...
export default class PosterPlugin extends UIContainerPlugin {
  ...
  update() {
    ...
    if (!this.hasStartedPlaying) {
      ...
      this.$el.show()
      let showPlayButton = !this.playRequested
      if (this.container.buffering) {
        // hide the play button whilst buffering
        // if autoplay is blocked (e.g. ios) but requested,
        // then this makes sure that the play button reappears after
        // the initial load.
        // (It is difficult/impossible to detect if autoplay will work
        // before this point)
